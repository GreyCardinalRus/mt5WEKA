...
input int ExtSpreads=5;
//input int ExtDeviation=5;
input int ExtBackstep=10;
...
//--- name for DataWindow and indicator subwindow label
   IndicatorSetString(INDICATOR_SHORTNAME,"ZigZag("+(string)ExtDepth+","+(string)ExtBackstep+")");
   ...
   for(shift=limit;shift<rates_total && !IsStopped();shift++)
     {
      ...
      if(val==lastlow) val=0.0;
      else
        {
         lastlow=val;
         //---
         if((low[shift]-val)>(spread[shift]*ExtSpreads*_Point)) val=0.0;
         else
           {
            //---
      ...
      if(val==lasthigh) val=0.0;
      else
        {
         lasthigh=val;
         //---
         if((val-high[shift])>(spread[shift]*ExtSpreads*_Point)) val=0.0;
         else
           {
            //---
            for(back=ExtBackstep;back>=1;back--)
